...
- Produce a plot to show the change in average `gdpPercap` over time.
...
```{r}
filter(rnai,!is.na(`FL/RL_rep1`)) %>% 
  mutate(`FL/RL_Av` = as.numeric(`FL/RL_Av`)) %>% 
  ggplot(aes(x=Row,y=Col,fill=`FL/RL_Av`)) + geom_tile() + facet_wrap(~Plate)
```


## Re-computing averages

```{r}
rnai_averages <- dplyr::select(rnai, Plate, Well,GeneSymbol,contains("rep")) %>% 
  gather(key = Sample,value=Value,-(Plate:GeneSymbol)) %>% 
  separate(Sample, into=c("Sample","Rep"),sep="_") %>% 
  group_by(Plate,Well,GeneSymbol,Sample) %>% summarise(Ave=mean(Value)) %>% 
  spread(Sample,Ave) 
```

```{r}
filter(rnai_averages,RL < 16) %>% 
  mutate(Label = ifelse(abs(`FL/RL`)>4,GeneSymbol,"")) %>% 
ggplot(aes(x = RL, y= FL,col=`FL/RL`,label=Label)) + geom_point(alpha=0.4)
```
...
```{r}
rnai <- left_join(rnai, anno, by="GeneID")
```
